...
Run `bun run release patch` (or `minor` or `major`) from a clean repository root. The release script updates the CLI version and Bun lockfile together, commits them, creates an annotated version tag, and pushes the commit and tag. The publish workflow runs the same checks, then publishes through npm with provenance.
