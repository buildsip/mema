  ...
  try {
    ...
    if (!migrations.length) {
      throw new MigrationError(
        "No bundled database migrations were found. Reinstall mema and retry mema init.",
      );
    }
    ...
    const schema = await client.query<{ exists: boolean }>(
      "SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'mema') AS exists",
    );
    let applied = 0;
    if (schema.rows[0]!.exists) {
      const journal = await client.query<{ name: string | null }>(
        "SELECT to_regclass('mema.__drizzle_migrations')::text AS name",
      );
      if (!journal.rows[0]!.name) {
        throw new MigrationError(
          "The mema database schema exists without its migration history. No migrations were applied. Restore its original mema.__drizzle_migrations table from backup, or configure a different database with no mema schema, then retry mema init. Do not delete existing tables to bypass this check.",
        );
      }
      const history = await client.query<{ hash: string; created_at: string }>(
        "SELECT hash, created_at FROM mema.__drizzle_migrations ORDER BY created_at, id",
      );
      ...
      for (const [i, entry] of history.rows.entries()) {
        ...
        if (
          ...
          throw new MigrationError(
            "The database migration history does not match this mema release; it may be newer or modified. No migrations were applied. Use the mema release that owns this history or a compatible newer release, then retry mema init. Do not edit the migration history or config.json version to bypass this check.",
          );
        }
      }
      ...
      if (!applied) {
        ...
        const objects = await client.query(
          "SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'mema' AND c.relkind IN ('r', 'p', 'v', 'm', 'S', 'f') AND c.relname NOT IN ('__drizzle_migrations', '__drizzle_migrations_id_seq') LIMIT 1",
        );
        if (objects.rows.length) {
          throw new MigrationError(
            "The mema schema has existing objects but no applied migration records. No migrations were applied. Restore the original migration history or configure a different database with no mema schema, then retry mema init.",
          );
        }
      }
    }
    ...
    throw new Error(
      "Database setup failed. Check that the credential command returns a reachable direct or session-pooled PostgreSQL URL and that its database role can create and alter objects in the mema schema. Check the database server logs for connection, permission, or SQL failures; another setup may hold the migration lock. Retry mema init after fixing the cause. Pending migrations are transactional; existing votes are not reset. Connection details are hidden to protect credentials.",
    );
  } finally {
