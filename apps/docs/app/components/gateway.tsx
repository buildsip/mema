...
export type GatewayFlowProps = {
  ...
  saturation?: number;
  brightness?: number;
  /** Width of the empty gap in the middle, in pixels. Left lines stop at its left edge, right lines at its right edge. */
  hole?: number;
  /** Vertical center of that gap, in pixels from the top of the frame. */
  focusY?: number;
  /** CSS color of the button the lines travel toward, usually `rgb(...)`. */
  buttonColor?: string;
  className?: string;
  style?: CSSProperties;
};
...
<body class="bg-black text-slate-300 antialiased min-h-screen flex flex-col selection:bg-slate-700 selection:text-white relative" style="font-family: 'Inter', sans-serif;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Reads an rgb() or rgba() string from the page. Null until the button has been measured.
            function parseRgb(color) {
                if (!color) return null;
                const open = color.indexOf('(');
                const close = color.indexOf(')', open);
            ...
            // 0 at the screen edge, 1 at the button. The shift waits until the
            // line is closing in, then eases into the button color.
            function approachMix(along) {
                const x = Math.min(1, Math.max(0, (along - 0.45) / 0.55));
                return x * x * (3 - 2 * x);
            }

            // along is 0 at the outer edge and 1 at the button.
            // Alpha rises with the mix so the color near the button matches the
            // button, not a faint tint of it on the black background.
            function lineColor(along, alphaFrom, alphaTo) {
                const controls = window.__SF_CONTROLS || {};
                const light = controls.mode === 'light';
                const base = light ? [26, 31, 42] : [255, 255, 255];
                const button = parseRgb(controls.buttonColor);
                // No button color yet: keep the original flat line.
                if (!button) {
                    return 'rgba(' + base[0] + ',' + base[1] + ',' + base[2] + ',' + alphaFrom + ')';
                }
                const mix = approachMix(along);
                const alpha = alphaFrom + (alphaTo - alphaFrom) * mix;
                const channel = (index) => Math.round(base[index] + (button[index] - base[index]) * mix);
                return 'rgba(' + channel(0) + ',' + channel(1) + ',' + channel(2) + ',' + alpha + ')';
            }

            // Horizontal, because the paths travel sideways into the button.
            // Stop 0 sits on the screen edge. Stop 1 sits on the button edge.
            function strokeGradient(fromX, toX, alphaFrom, alphaTo) {
                if (fromX === toX) return lineColor(1, alphaFrom, alphaTo);
                const gradient = ctx.createLinearGradient(fromX, 0, toX, 0);
                [0, 0.45, 0.62, 0.78, 0.9, 1].forEach((stop) => {
                    gradient.addColorStop(stop, lineColor(stop, alphaFrom, alphaTo));
                });
                return gradient;
            }
            ...
            function render() {
                ...
                const controls = window.__SF_CONTROLS || {};
                // hole is the button width. focusY is the button's vertical center.
                // The page measures the button and sends both in. This frame cannot see it.
                // Left lines end on the button's left edge. Right lines end on its right edge.
                // They never cross that gap: every control point stays on its own side.
                const hole = controls.hole || 0;
                const centerY = controls.focusY > 0 ? controls.focusY : height / 2;
                const light = controls.mode === 'light';
                const lineAlpha = light ? 0.4 : 0.35;
                const dotAlpha = light ? 0.75 : 0.7;
                const leftEnd = centerX - hole / 2;
                const rightEnd = centerX + hole / 2;
                const leftStroke = strokeGradient(0, leftEnd, lineAlpha, 0.9);
                const rightStroke = strokeGradient(width, rightEnd, lineAlpha, 0.9);
                ...
                paths.forEach(path => {
                    const edgeX = path.isLeft ? 0 : width;
                    const endX = path.isLeft ? centerX - hole / 2 : centerX + hole / 2;
                    const p0 = { x: edgeX, y: path.startY };
                    const p1 = { x: edgeX + (endX - edgeX) * 0.45, y: path.startY };
                    const p2 = { x: edgeX + (endX - edgeX) * 0.82, y: centerY };
                    const p3 = { x: endX, y: centerY };

                    ctx.beginPath();
                    ctx.moveTo(p0.x, p0.y);
                    ctx.bezierCurveTo(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
                    ctx.strokeStyle = path.isLeft ? leftStroke : rightStroke;
                    ctx.lineWidth = 1.2;
                    ctx.setLineDash([1, 4]);
                    ctx.stroke();
                    ctx.setLineDash([]);
                    ...
                    path.particles.forEach(p => {
                        ...
                        const span = endX - edgeX;
                        const along = span === 0 ? 1 : (pos.x - edgeX) / span;
                        ...
                        ctx.fillStyle = lineColor(along, dotAlpha, 1);
...
function GatewayFlowFrame({
  ...
  const safeHole = clamp(hole, 0, 4000);
  const safeFocusY = clamp(focusY, 0, 4000);
  const safeButtonColor = /^rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+/.test(buttonColor) ? buttonColor : "";
  ...
  // Hole and focus stay live. Rebuilding the frame for them would restart the animation.
  const postControls = () => {
